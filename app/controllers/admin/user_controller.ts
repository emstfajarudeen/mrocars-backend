...
async function findAdminUser(id: number) {
  return User.query().where('id', id).where('role', 'user').first()
}
...
export default class UserController {
  ...
  async toggleStatus({ params, response }: HttpContext) {
    const user = await findAdminUser(Number(params.id))
    ...
    return ApiResponse.success(
      response,
      { message: 'User status updated', is_active: user.isActive },
      'User status updated'
    )
  }
